...
paramGrid <- expand.grid(c(10), c(10), seq(-0.5, 0.5, 0.2), c(200, 300, 400, 500))
...
saveRDS(optimizerResults, file = "optimizerResults_Buy_BP_Algo")
# objToOptimize(paramGrid[1,])

###################################################################
# optimizerResults <- readRDS(file = "optimizerResults_Buy_BP_Algo")
# optimizerResults_df <- do.call(rbind.data.frame, optimizerResults)
# colnames(optimizerResults_df) <- c("Objective", "max_drift_over_participating", "max_drift_under_participating", "BP_thresh")
